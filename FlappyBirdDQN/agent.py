...
class Agent:
    def __init__(self, hyperparameter_set):
        ...
        self.stop_on_reward_max = hyperparameters["stop_on_reward_max"]
        self.stop_on_reward_min = hyperparameters["stop_on_reward_min"]
    ...
    def run(self, is_training=True, render=False):
        ...
        for episode in itertools.count():
            ...
            while (
                not terminated
                and not truncated
                and reward_sum < self.stop_on_reward_max
                and reward_sum > self.stop_on_reward_min
            ):
            ...
            if is_training:
                if reward_sum > best_reward:
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: New best reward: {reward_sum:.01f}"
                    print(log_message)
    ...
    def save_graph(self, reward_history, epsilon_history):
        ...
        for x in range(len(mean_rewards)):
            mean_rewards[x] = np.mean(reward_history[max(0, x - 99) : (x + 1)])
        plt.subplot(121)
